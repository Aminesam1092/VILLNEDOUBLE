gsap.to(".OURSTORY__Headingtext",{
    scrollTrigger: {
        trigger: ".OURSTORY__Headingtext",
        start: "top center",
        end: '+=2000',
        scrub: true,
        pin: true,
    },
    opacity: 1,
});

gsap.to(".OURSTORY__Catchcopy",{
    scrollTrigger: {
        trigger: ".OURSTORY__Catchcopy",
        start: "top center",
        scrub: true,
    },
    duraion: 5,
    opacity: 1,
});

gsap.to(".OURSTORYconteiner__Twocontents",{
    scrollTrigger: {
        trigger: ".OURSTORYconteiner__Twocontents",
        start: "top center",
    },
    duraion: 1,
    opacity: 1,
});

gsap.to(".OURSTORYconteiner__Establishedtime",{
    scrollTrigger: {
        trigger: ".OURSTORYconteiner__Establishedtime",
        start: "top center",
        scrub: true,
    },
    duraion: 5,
    opacity: 1,
});

gsap.to(".OURSTORYconteiner__Logo",{
    scrollTrigger: {
        trigger: ".OURSTORYconteiner__Logo",
        start: "top center",
        scrub: true,
    },
    duraion: 5,
    opacity: 1,
});

gsap.to(".OURSTORYconteiner__Brandeintroductory__Brandename",{
    scrollTrigger: {
        trigger: ".OURSTORYconteiner__Brandeintroductory__Brandename",
        start: "top center",
        end: '+=1000',
        scrub: true,
        pin: true,
    },
    duraion: 5,
    opacity: 1,
});

gsap.to(".OURSTORYconteiner__Brandeintroductory",{
    scrollTrigger: {
        trigger: ".OURSTORYconteiner__Brandeintroductory",
        start: "top center",
    },
    duraion: 5,
    opacity: 1,
});

gsap.to(".OURSTORYconteiner__Brandeintroductory__Text",{
    scrollTrigger: {
        trigger: ".OURSTORYconteiner__Brandeintroductory__Text",
        start: "top center",
    },
    duraion: 5,
    opacity: 1,
});

gsap.to(".Productconteiner__Image",{
    scrollTrigger: {
        trigger: ".Productconteiner__Image",
        start: "top center",
    },
    duraion: 5,
    opacity: 1,
})

gsap.to(".Productconteiner__Text",{
    scrollTrigger: {
        trigger: ".Productconteiner__Text",
        start: "top center",
        end: "+=500",
        scrub: true,
    },
    duraion: 5,
    opacity: 1,
})


